...
const INACCURATE_DATA_ERROR = require('../utils/errors/InaccurateDataError');
const UNAUTHORIZED_ERROR = require('../utils/errors/UnauthorizedError');
const NOT_FOUND_ERROR = require('../utils/errors/NotFoundError');
const CONFLICT_ERROR = require('../utils/errors/ConflictError');
    ...
    .then(({ _id }) => {
      if (_id) {
        const token = jwt.sign(
          { _id },
          NODE_ENV === 'production' ? SECRET_SIGNING_KEY : 'secretKey',
          { expiresIn: '7d' },
        );
